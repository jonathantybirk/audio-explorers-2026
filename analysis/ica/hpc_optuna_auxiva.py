"""
Optuna hyperparameter optimisation for AuxIVA on mixture.wav.

Searches over:
  stft_size  : {512, 1024, 2048}
  hop_size   : stft_size // {2, 4}
  n_iter     : 20–150
  proj_back  : True / False  (projection-back normalisation)

Objective: minimise sum of |off-diagonal| cross-correlations across 4
separated sources (lower = more independent).

Outputs:
  analysis/ica/optuna/best_params_auxiva.json
  analysis/ica/separated/mixture_auxiva_opt_source_{1..4}_{az}deg.wav
"""
...
def auxiva_separate(data, stft_size, hop_size, n_iter, proj_back, wiener):
    """Returns (n_samples, 4) mono sources."""
    ...
    if wiener:
        Y = pra.bss.auxiva(X, n_iter=n_iter, proj_back=proj_back)
        W_hat = pra.bss.project_back(Y, X[:, :, 0])
        # Wiener post-filter: ratio mask per source
        powers = np.abs(Y) ** 2            # (frames, freqs, n_src)
        total  = powers.sum(axis=-1, keepdims=True) + 1e-8
        masks  = powers / total
        Y_filtered = Y * masks
        separated = Y_filtered
    else:
        Y = pra.bss.auxiva(X, n_iter=n_iter, proj_back=proj_back)
        separated = Y
    ...
    sources = []
    for k in range(separated.shape[2]):
        mono = pra.transform.stft.synthesis(separated[:, :, k], stft_size, hop_size,
                                            win=synthesis_win)
...
def objective(trial):
    ...
    wiener     = trial.suggest_categorical("wiener",     [True, False])

    try:
        mono = auxiva_separate(data, stft_size, hop_size, n_iter, proj_back, wiener)
...
best_path = os.path.join(OPT_DIR, "best_params_auxiva.json")
