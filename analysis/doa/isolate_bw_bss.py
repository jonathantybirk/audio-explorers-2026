"""
Apply Brunch woman inter-ear mask to all 4 channels, then run FastMNMF2 n=2.
Brunch woman is right-dominant (left_ratio < 0.5 - THRESH).
"""
import os
import numpy as np
...
bw_mask    = (left_ratio < (0.5 - THRESH)).astype(np.float64)

print(f"  Brunch woman mask: {bw_mask.mean()*100:.1f}% of TF bins kept")

print("Applying mask to all 4 channels ...")
cleaned = np.zeros((N, 4), dtype=np.float64)
for ch in range(4):
    _, sig = istft(X[ch] * bw_mask, fs=sr, nperseg=NPERSEG, noverlap=NOVERLAP)
...
for n_src in [3, 4]:
    ...
    Y_all = pra.bss.fastmnmf2(X_bss.copy(), n_src=n_src, n_iter=N_ITER,
    ...
    for k in range(n_src):
        ...
        save_wav(os.path.join(SEP_DIR, f"brunch_woman_bss_n{n_src}_source_{k+1}.wav"), mono, sr)
